...
function SwapFaceDetail() {
  ...
  const fetchImages = async () => {
    const { data } = await axios.get(`https://api.funface.online/get/list_image/all_santa/12?page=${currentPage}`);
    if (data) {
      setResponse(data);
    }
  };
      ...
      <div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 justify-center px-5 mt-6">
          {response && response.list_sukien_video.map(item => (
            <ProductCard key={item.id} image={item.image} link={`/swap-face/${item.id}?album_id=${item.IDCategories}`} title={item.thongtin} />
          ))}
        </div>
